...
\begin{itemize}
  ...
  \item 集合 $A$ の\emph{冪集合\emphp{power set}}を $\Power(A) = \{X \mid X \subseteq A\}$，有限冪集合を $\PowerFin(A) = \{X \in \Power(X) \mid \text{$X$ は有限集合}\}$ と表記する．
  ...
  \item 集合 $A$ の $B$ との\emph{差集合\emphp{relative complement}}を $A \rcomplement B = \{a \in A \mid a \not\in B\}$ と表記する．
\end{itemize}
...
\begin{definition}[ランク付きアルファベット (ranked alphabet)]
...
\begin{definition}[項代数 (term algebra)]
...
\begin{definition}[コンテキスト (context)]
