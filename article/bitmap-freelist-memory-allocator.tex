...
\begin{center}
  \begin{tabular}{|p{0.9\textwidth}|}
    ...
    \begin{algorithmic}[1]
      ...
      \If{$\mathit{cls} = -1$}
        \State{$\mathit{blk} \assign (\mathrm{Free}, \mathrm{allocFreeSize}(\mathit{size}))$}
      \Else
        ...
        \Else
          ...
          \State{$\mathrm{free}(V_\mathit{cls}) \assign |\mathrm{segments}(H)|$}
          \State{$i_\mathrm{seg} \assign \mathrm{free}(V_\mathit{cls})(1)$}
          \State{$\mathrm{segments}(H)(i_\mathrm{seg}) \assign \mathrm{newSegment}(\mathit{cls})$}
          \State{$\mathit{seg} \assign \mathrm{segments}(H)(i_\mathrm{seg})$}
          \State{$i_\mathrm{blk} \assign 1$}
          \State{$\mathrm{bitmap}(\mathit{seg})(i_\mathrm{blk}) \assign \top$}
        \EndIf
          \State{$\mathit{blk} \assign (\mathrm{Fixed}, i_\mathrm{seg}, i_\mathrm{blk})$}
      \EndIf
    \end{algorithmic}
    \\\hline
  \end{tabular}
...
\begin{center}
  \begin{tabular}{|p{0.9\textwidth}|}
    ...
    \begin{algorithmic}[1]
      ...
      \Ensure{$H$}
      \If{$\mathit{blk} = (\mathrm{Free}, \mathit{body})$}
        \State{$\mathrm{freeFreeSize}(\mathit{blk})$}
      \ElsIf{$\mathit{blk} = (\mathrm{Fixed}, i_\mathit{seg}, i_\mathit{blk})$}
        \State{$\mathit{seg} \assign \mathrm{segments}(H)(i_\mathrm{seg})$}
        \State{$\mathit{cls} \assign \mathrm{subheapClass}(\mathit{seg})$}
        \State{$V_\mathit{cls} \assign \mathrm{subheap}_{\mathit{cls}}(H)$}
        \If{$\forall i \in [\mathrm{blockCountOfClass}(\mathit{cls})], i \neq i_\mathrm{blk}\ldotp \mathrm{bitmap}(\mathit{seg})(i) = \top$}
          \State{$\mathrm{free}(V_\mathit{cls}) \assign \mathrm{free}(V_\mathit{cls}) \cdot i_\mathrm{seg}$}
        \EndIf
        ...
        \If{$\forall i \in [\mathrm{blockCountOfClass}(\mathit{cls})]\ldotp \mathrm{bitmap}(\mathit{seg})(i) = \bot$}
          ...
          \State{$\mathrm{free}(H) \assign \mathrm{free}(H)\cdot i_\mathrm{seg}$}
        \EndIf
      \EndIf
    \end{algorithmic}
    \\\hline
